...
/**
 * Java Extension equivalent of template-SMART-OUTBOUND-03.js
 *
 * <p>This extension demonstrates outbound processing with transportFields:</p>
 * <ul>
 *   <li>Transforming Cumulocity measurement to device message</li>
 *   <li>Adding transport-specific fields (e.g., Kafka record key)</li>
 *   <li>Useful for Kafka connectors requiring message keys</li>
 * </ul>
 *
 ...
 * </pre>
 *
 * <p>Output (device message with transportFields):</p>
 * <pre>
 ...
 * }
 * TransportFields: {
 *   "key": "12345"  // Kafka record key
 * }
 * </pre>
 */
@Slf4j
public class ProcessorExtensionSmartOutbound03 implements ProcessorExtensionOutbound<Object> {
    ...
    public DeviceMessage[] onMessage(Message<Object> message, JavaExtensionContext context) throws ProcessingException {
        try {
            ...
            // Note: transportFields is used to add Kafka-specific fields like "key"
            return new DeviceMessage[] {
                DeviceMessage.forTopic("measurements/" + sourceId)
                    .payload(jsonPayload)
                    .transportField("key", sourceId)  // Add Kafka record key
                    .build()
            };
