...
@Injectable()
export class StatisticTabFactory implements TabFactory {
  constructor(
  ...
  async get() {
    // console.log("StatisticTabFactory",this.router.url, this.router.url.match(/c8y-pkg-dynamic-mapper/g));
    const feature = await this.sharedService.getFeatures();

    const tabs: Tab[] = [];
    if (this.router.url.match(/c8y-pkg-dynamic-mapper\/node2\/monitoring\/statistic/g)) {
      tabs.push({
