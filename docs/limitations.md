...
2. **Smart Functions** — JavaScript callbacks executed in a GraalVM polyglot sandbox at runtime. Written in TypeScript using `dynamic-mapper-smart-function` for type safety, then compiled to JavaScript and pasted into the mapping editor. See the in-app documentation ([Smart Functions](../dynamic-mapper-ui/public/docs/smartfunction.md)) for details.
