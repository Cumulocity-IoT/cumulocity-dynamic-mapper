...
| `subscribe(String topic, Qos qos)` (protected) | Subscribe to one topic. The `qos` handed in is already clamped to the connector's `supportedQos` — see [`reliability.md`](reliability.md). |
